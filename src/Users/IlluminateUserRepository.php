...
use Carbon\Carbon;
use myGedung\Auth\Hashing\HasherInterface;
...
class IlluminateUserRepository implements UserRepositoryInterface
{
     ...
     * The hasher instance.
     *
     * @var \myGedung\Auth\Hashing\HasherInterface
     */
    protected $hasher;
     ...
     * @var string
     */
    protected $model = 'myGedung\Auth\Users\EloquentUser';

    /**
     * Create a new Illuminate user repository.
     *
     * @param  \myGedung\Auth\Hashing\HasherInterface  $hasher
    ...
    public function create(array $credentials, Closure $callback = null)
    {
        ...
        $this->fireEvent('auth.user.creating', compact('user', 'credentials'));
        ...
        $this->fireEvent('auth.user.created', compact('user', 'credentials'));
    ...
    public function update($user, array $credentials)
    {
        ...
        $this->fireEvent('auth.user.updating', compact('user', 'credentials'));

        $this->fill($user, $credentials);

        $user->save();

        $this->fireEvent('auth.user.updated', compact('user', 'credentials'));
    ...
    {
        $this->fireEvent('auth.user.filling', compact('user', 'credentials'));
        ...
        $this->fireEvent('auth.user.filled', compact('user', 'credentials'));
    }

    /**
     * Returns the hasher instance.
     *
     * @return \myGedung\Auth\Hashing\HasherInterface
     */
